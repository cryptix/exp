...
func (v *App) InitChildren() {

	// Get existing todos
	args := &types.TodoListArgs{"All"}
	var todos []types.Todo
	if err := v.Client.Call("TodoService.List", args, &todos); err != nil {
		panic(err)
	}
...
func (v *App) newTodoKeyUp(event dom.Event) {
	...
	var t types.Todo
	t.Title = title
	if err := v.Client.Call("TodoService.Save", t, &t); err != nil {
		panic(err)
	}
	todoView := &Todo{
		Model:  &t,
